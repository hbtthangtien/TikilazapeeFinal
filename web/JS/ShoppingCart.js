...
let totalPrice = 0;
span_bottom.innerHTML = '(' + chk_product_box_origin.length + ')';
chk_select_all.addEventListener('click', () => {
    ...
    if (status) {
        numberItemSelected = chk_product_box.length;
...
function changeQuantity(btn, val, unit_price, id, cart_id) {
    let quantity;
    //console.log(val);
    if (Number(val) > 0) {
...
btn_checkout.addEventListener('click', () => {
    ...
    const items_id = Array.from(chk_product_box_origin1).filter(rb => rb.checked === true);
