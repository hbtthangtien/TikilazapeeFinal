...
public class AdminManageUser extends HttpServlet {
    ...
    throws ServletException, IOException {
        ...
        List<User> listUser = ub.getListUserForAdmin();
         request.setAttribute("listUser", listUser);
                 request.getRequestDispatcher("view/AdminView/adminManageUser.jsp").forward(request, response);
    } 
    ...
    throws ServletException, IOException {
        ...
        if(valueOption  == 0){
            user = db.getListUserForAdmin();
        }
        else if(valueOption == 1){
            user = db.getListUserForAdminAZ();
        }
        else{
            user = db.getListUserForAdminZA();
        }
